#!/usr/bin/env node
/**
 * Worker Pool Streaming Stress Test
 * 
 * Tests the WorkerPoolBatcher for true parallel concurrent transcription.
 * Uses multiple worker threads, each with its own Engine for true parallelism.
 * 
 * Run with: node stress-tests/worker-pool-streaming.test.js [options]
 */
...
async function main() {
  ...
  console.log('  WORKER POOL STREAMING STRESS TEST');
  console.log('═'.repeat(70));
  ...
  const progressInterval = setInterval(() => {
    ...
    process.stdout.write(`\r  Progress: ${completed}/${CONCURRENT_STREAMS} (${pct}%) | Chunks: ${stats.totalChunks} | Pending: ${stats.pendingRequests}`);
  }, 100);
...
function analyzeResults(results, totalTestTime, poolStats) {
  console.log('═'.repeat(70));
  ...
  // Check 4: Throughput > numWorkers * 0.5
  const minThroughput = poolStats.workersReady * 0.5;
  if (throughputRatio >= minThroughput) {
    checks.push({ pass: true, msg: `Throughput ${throughputRatio.toFixed(2)}x >= ${minThroughput.toFixed(1)}x (good worker utilization)` });
  } else {
    checks.push({ pass: false, msg: `Throughput ${throughputRatio.toFixed(2)}x < ${minThroughput.toFixed(1)}x (poor worker utilization)` });
  }
  ...
  console.log('\n  ' + (allPassed ? '🎉 ALL CHECKS PASSED - True concurrent streaming works!' 
